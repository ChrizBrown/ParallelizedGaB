...
int main(int argc, char * argv[])
{
  ...
  FILE* res = fopen("iter_experiment_result.txt", "w");
  FILE* res2 = fopen("iter_experiment_result.csv","w");
  fprintf(res2,"NbIter,Time (ms)\n");
  int nb_hold = NbIter;
  for (int run = 100; run <= nb_hold; run+=100) 
  {
  NbIter = run;
  ...
  fprintf(res, "nbiter %d: alpha: %1.5f BER: %d time average: %f\n",run,alpha,NbBitError,timeAveragePerNb);
  if (alpha < 0.03f)
    fprintf(res2, "%d,%f\n",run,timeAveragePerNb);
}
fprintf(res,"\n");
fprintf(res2,"\n");
// Stop our streams
