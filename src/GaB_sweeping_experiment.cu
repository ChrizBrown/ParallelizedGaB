...
int main(int argc, char * argv[])
{
  ...
  int hold_Stream = num_streams;
  int batch_hold = batchSize;
  for (int run = 1; run <= hold_Stream; run++) 
  {
  for (int batcher = 1; batcher <= batch_hold; batcher++)
  {
  num_streams = run;
  batchSize = batcher;
  ...
  cudaMemcpyToSymbol(device_RowDegree, RowDegree, sizeof(int)*M,0,cudaMemcpyHostToDevice);
  ...
  cudaMemcpyToSymbol(device_numBrow, numBrow, sizeof(int)*M,0,cudaMemcpyHostToDevice);
  ...
  for (int stream_cnt = 0; stream_cnt < num_streams; stream_cnt++)
  {
    ...
    for (iter=0;iter<NbIter;iter++)
    {
      ...
      // Call Decode
      global_decode_batched_stream<<<gridDim,blockDim,0,pStreams[stream_cnt]>>>(device_VtoC,device_CtoV,device_Mat,
  ...
  fprintf(res, "stream %d: batch size: %d alpha: %1.5f BER: %d time average: %f\n",run, batcher,alpha,NbBitError,timeAveragePerNb);
  fprintf(res2, "%1.5f,%d,%d,%f,,",alpha,run, batcher,timeAveragePerNb);
}
fprintf(res,"\n");
fprintf(res2,"\n");
// Stop our streams
