...
__global__ void global_decode_batched_stream(int *VtoC,int *CtoV,int** Mat,int* RowDegree,int* ColumnDegree,
                              int* Decide,int *Receivedword,int *Interleaver,int M,int N,
                              int *numBrow,int *numBcol, int iter, int* isCodeWord, int batchSize, int NbBranch)
{                          
...
__global__ void global_decode_batched_stream_shared(int *VtoC,int *CtoV,int** Mat,int* RowDegree,int* ColumnDegree,
                              int* Decide,int *Receivedword,int *Interleaver,int M,int N,
                              int *numBrow,int *numBcol, int iter, int* isCodeWord, int batchSize, int NbBranch){                          
  ...
  for(int batchIdx=0;batchIdx<batchSize;batchIdx++)
  {
    ...
    if(iter==0){
      DataPassGBIter0_(VtoC+batchRelationOffset,CtoV+batchRelationOffset,Receivedword_shared+batchDataOffset,Interleaver,ColumnDegree,N,numBcol);
    }
    else{
      DataPassGB_(VtoC+batchRelationOffset,CtoV+batchRelationOffset,Receivedword_shared+batchDataOffset,Interleaver,ColumnDegree,N,numBcol);
    }
    __syncthreads();
    ...
    APP_GB_(Decide_shared+batchDataOffset,CtoV+batchRelationOffset,Receivedword_shared+batchDataOffset,Interleaver,ColumnDegree,N,M,numBcol);
    __syncthreads();  

    if(ComputeSyndrome_(Decide_shared+batchDataOffset,Mat,RowDegree,M) == 0){
      isCodeWord[batchIdx] = 0;
    }
  }
  if (idx < N*batchSize)
